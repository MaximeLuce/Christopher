        ...
        connection.query(
            ...
            async function(err, results) {
                ...
                const statsday = new MessageEmbed()
                    ...
                    .addField('Bonnes réponses :', `${results[0].vrai} réponses (${results[0].vrai/total*100}%)`)
                    .addField('Mauvaises réponses :', `${results[0].faux} réponses (${results[0].faux/total*100}%)`)
                    .addField('Temps imparti dépassé :', `${results[0].timeout} réponses (${results[0].timeout/total*100}%)`)
                    .setColor('#3867d6')
        ...
        connection.query(
            ...
            async function(err, results) {
                ...
                const stats = new MessageEmbed()
                    ...
                    .addField('Nombre de questions :', total)
                    .addField('Bonnes réponses :', `${results[0].vrai_total} réponses (${(results[0].vrai/total*100).toFixed(2)}%)`)
                    .addField('Mauvaises réponses :', `${results[0].faux_total} réponses (${(results[0].faux/total*100).toFixed(2)}%)`)
                    .addField('Temps imparti dépassé :', `${results[0].timeout_total} réponses (${(results[0].timeout/total*100).toFixed(2)}%)`)
                    .setColor('#3867d6')
                    .attachFiles(['assets/images/logo.png'])
                    .setAuthor('Le Max de Culture', 'attachment://logo.png', 'https://le-max-de-culture.fr/')
                    .setTimestamp()
